...
byte			UTFT___p1, UTFT___p2, UTFT___p3, UTFT___p4, UTFT___p5;
...
#include "hardware/stm8/HW_XH18LCD.h"


#include "memorysaver.h"


void UTFT(byte model, int RS, int WR, int CS, int _RST)
{
  UTFT_SER(model, RS, WR, CS, _RST, 0);
}

void UTFT_SER(byte model, int RS, int WR, int CS, int _RST, int SER)
{ 
...
{   
	if (UTFT_display_transfer_mode!=1)
	{
		...
		UTFT_LCD_Writ_Bus(0x00,VL,UTFT_display_transfer_mode);
	}
	else
		UTFT_LCD_Writ_Bus(0x00,VL,UTFT_display_transfer_mode);
}
...
void UTFT_LCD_Write_DATA(char VH,char VL)
{
	if (UTFT_display_transfer_mode!=1)
	{
		...
		UTFT_LCD_Writ_Bus(VH,VL,UTFT_display_transfer_mode);
	}
	else
	{
		UTFT_LCD_Writ_Bus(0x01,VH,UTFT_display_transfer_mode);
		UTFT_LCD_Writ_Bus(0x01,VL,UTFT_display_transfer_mode);
	}
}
...
void UTFT_clrScr()
{
	...
	else
	{
		...
		{
			if (UTFT_display_transfer_mode!=1)
				UTFT_LCD_Writ_Bus(0,0,UTFT_display_transfer_mode);
			else
			{
				UTFT_LCD_Writ_Bus(1,0,UTFT_display_transfer_mode);
				UTFT_LCD_Writ_Bus(1,0,UTFT_display_transfer_mode);
			}
		}
	}
...
void UTFT_fillScr(word color)
{
	...
	else
	{
		...
		{
			if (UTFT_display_transfer_mode!=1)
				UTFT_LCD_Writ_Bus(ch,cl,UTFT_display_transfer_mode);
			else
			{
				UTFT_LCD_Writ_Bus(1,ch,UTFT_display_transfer_mode);
				UTFT_LCD_Writ_Bus(1,cl,UTFT_display_transfer_mode);
			}
		}
	}
	sbi(UTFT_P_CS, UTFT_B_CS);
